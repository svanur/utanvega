...
function EditionDialogInner({ open, edition, eventId, onClose, onSaved, onGalleryMutated, onNotify, initialValues, isClone = false, siblingEditions }: EditionDialogProps) {
  ...
  // uniqueness constraint by design; same-year reruns/reschedules are legitimate). Recomputed
  // on every render so it tracks form.title live without needing to close/reopen the dialog.
  ...
  const duplicateTitleMatch = trimmedTitle
    ? siblingEditions.find(sibling =>
        sibling.id !== edition?.id
        && (sibling.title ?? '').trim().toLowerCase() === trimmedTitle.toLowerCase()
      )
    : undefined;
        ...
        <Stack spacing={2} sx={{ mt: 1 }}>
          ...
          {duplicateTitleMatch && (
            <Alert severity="warning">
              Another edition of this event is already titled &quot;{duplicateTitleMatch.title}&quot;
            </Alert>
          )}
    ...
    >
      ...
      <TableCell onClick={e => e.stopPropagation()}>
        <Stack direction="row" alignItems="center" spacing={0.5}>
          ...
          {edition.status === 'Completed' && race.status !== 'Completed' && race.status !== 'Cancelled' && (
            <Tooltip title="Edition is Completed but this race is not — update via the race status chip">
...
export default function EventDetailPage({ onNotify, onNavigateToRaceManager }: EventDetailPageProps) {
  ...
  const [completingEditionId, setCompletingEditionId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (!completingEditionId) return;
    const t = setTimeout(() => setCompletingEditionId(null), 3000);
    return () => clearTimeout(t);
  }, [completingEditionId]);
  ...
  const handleCompleteEdition = async (edition: EventEditionDto) => {
    if (completingEditionId !== edition.id) {
      setCompletingEditionId(edition.id);
      return;
    }
    try {
  ...
  const handleCycleEditionStatus = (edition: EventEditionDto) => {
    ...
    if (edition.status === 'Cancelled' || edition.status === 'Completed') return;
              ...
              <Stack direction="row" spacing={0.5} alignItems="center" onClick={e => e.stopPropagation()}>
                ...
                <Tooltip title={edition.status === 'Cancelled'
                  ...
                  : edition.status === 'Completed'
                    ? 'Completed — reactivate via Edit edition'
