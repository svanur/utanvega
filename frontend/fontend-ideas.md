# Idea bank for frontend

## Need to explore and plan better

- [ ] Add 'Load more' button to trail list
- [ ] Offline Map Support: Cache GeoJSON and map tiles locally.
- [ ] Push Notifications: Notify users on new trails or condition changes.
- [ ] Long Press for Quick View: Show elevation/info preview on hold.

## Backlog

- [x] Convert to PWA for home-screen installation and better mobile experience.
- [ ] GPS "Follow Me" mode on `TrailMapView`.
- [ ] Off-trail vibration alerts using GPS tracking.
- [ ] "Report Condition" mobile UI for real-time trail status.
- [ ] System Share Sheet integration using `navigator.share()`.

## Done

- [x] Add total count of trails
- [x] Filter for "Favorite trails" in trail list.
- [x] Implement "Swipe to Favorite" with local storage persistence.
- [x] Move Location chip to the top row next to ActivityType in trail list
- [x] Use location slug for links on trail details page
- [x] Display all trails in location as pins on location details map
- [x] Add location page with all locations and trails
- [x] Add location info
- [x] Display map, pin for each trail
- [x] random quotes
- [x] Advanced search
- [x] Add simple search to trail list
- [x] Display TrailType
- [x] Display details trail page
- [?] Open in Garmin connect
- [x] Share trail
- [x] Display simple Trail list
