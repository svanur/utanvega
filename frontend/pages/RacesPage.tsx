...
export default function RacesPage({ mode, onToggleMode, showQuote = false }: RacesPageProps) {
    ...
    const filtered = useMemo(() => {
        ...
        const result = applyFilters(visible, filters, q, favoriteEvents, includeAllEvents);
    ...
    }, [events, search, filters, loc, favoriteEvents, includeAllEvents]);
    ...
    const hasMoreEventsIncludingPast = useMemo(() => {
        ...
        if (!q) return false;
        const visible = events.filter(e => e.status !== 'Hidden' && e.status !== 'Unlisted');
        const futureOnly = applyFilters(visible, filters, q, favoriteEvents, false);
        const includingPast = applyFilters(visible, filters, q, favoriteEvents, true);
        return includingPast.length > futureOnly.length;
    }, [events, search, filters, favoriteEvents]);
    ...
    const monthsWithEvents = useMemo(() => {
        const q = search.toLowerCase().trim();
        const visible = events.filter(e => e.status !== 'Hidden' && e.status !== 'Unlisted');
        const result = applyFilters(visible, filters, q, favoriteEvents, includeAllEvents, true);
        return new Set(result.map(e => e.displayDate ?? e.nextEditionDate).filter(Boolean).map(d => new Date(d! + 'T00:00:00').getMonth()));
    }, [events, search, filters, favoriteEvents, includeAllEvents]);
